...
<%
    ...
    int labId = LaboratorioDAO.obtenerIdLaboratorioGoodness();
    String mensaje = "";
...
<head>
  ...
  <title>Goodness Lab - Máquina CTF</title>
...
<header>
  <div class="header-left">
    <h1 class="typewriter" data-text="GOODNESS MACHINE"></h1>
  </div>
  <div class="window-buttons">
    <button class="minimize">🗕</button>
    <button class="maximize">🗖</button>
    <button onclick="window.location.href='<%= request.getContextPath() %>/ovalabs/home_directory_ovalabs/home_ovalabs.jsp?page=0'" class="close">✖</button>
  </div>
</header>
...
<main>
    ...
    <div id="popupMessage" class="message-popup">
        ...
        <button class="message-popup-close-btn"><a href="<%= request.getContextPath() %>/ovalabs/labs/goodness.jsp" style="text-decoration: none; color: white;">Cerrar</a></button>
    </div>
    <br>
	<!-- POPUP MODAL -->
		<div id="labPopup-modal" class="labPopup-modal">
		    <div class="labPopup-content">
		        ...
		        <div class="labPopup-wrapper">
		            ...
		            <form class="labPopup-form" action="<%= request.getContextPath() %>/WriteupControladorOvalabs" method="post">
		            ...
		            <form class="labPopup-form" action="<%= request.getContextPath() %>/validarFlagOvalabs" method="get">
		                <input type="hidden" name="user_id" value="<%= usuarioJWT.getUserId() %>">
		                <input type="hidden" name="lab_id" value="<%= labId %>">
  ...
  <section class="lab-intro">
    <div class="lab-img">
      <img src="<%= request.getContextPath() %>/img/logo-test-15-update.png" alt="Máquina CTF">
    </div>
    <div class="lab-info">
      ...
      <p style="text-shadow: 1px 1px 2px gray;"><strong>Descripción:</strong> Aprende a sacar credenciales, fuerza bruta, fuzzing en la web, etc...</p>
      <p style="text-shadow: 1px 1px 2px gray;"><strong>Fecha de creación:</strong> 15/05/2025</p>
      <p style="text-shadow: 1px 1px 2px gray;"><strong>Plataformas:</strong> <img alt="VMWare" src="<%= request.getContextPath() %>/img/img_dockerpwned/vmware.png" width="30px" height="30px" class="img-plataform"></p>
  ...
  <section class="lab-tecnica">
    ...
    <ul class="tecnica-grid">
      ...
      <li>🪪 Credenciales SSH</li>
      <li>📄 Fuzzing Web</li>
      <li>🔨 Fuerza Bruta</li>
      <li>🔑 Privilegios SUDO</li>
    </ul>
  </section>
  ...
  <div class="botones">
	  ...
	  <button data-text="Ver Writeups"><a href="<%= request.getContextPath() %>/verWriteupsOvalabs?lab_id=<%= labId %>">ㅤㅤㅤㅤㅤㅤ</a></button>
...
<footer>
  <p>© 2025 Ovalabs Labs. Todos los derechos reservados.</p>
</footer>
...
<script>
    ...
    function closePopup() {
        ...
        window.location.href = "<%= request.getContextPath() %>/ovalabs/labs/goodness.jsp"; // Redirigimos
    }
  ...
  document.addEventListener("DOMContentLoaded", () => {
	...
	// ENVIAR PETICION PARA OBTENER TODAS LAS FLAGS DE ROOT
	  ...
	  document.addEventListener("DOMContentLoaded", function () {
	    ...
	    fetch(`<%= request.getContextPath() %>/getFlagRootOvalabsCount?lab_id=<%= labId %>`)
	        .then(response => {
	            if (!response.ok) {
	...
	// OBTENER LA INFORMACION DE LOS LOGS DE LOS WRITEUPS
	  ...
	  window.addEventListener('DOMContentLoaded', () => {
	      fetch('<%= request.getContextPath() %>/FlagLogOvalabsControlador?lab_id=<%= labId %>')
